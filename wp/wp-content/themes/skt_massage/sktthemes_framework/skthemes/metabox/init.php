 ...
 */
define('CMB_META_BOX_URL', get_template_directory_uri() . '/sktthemes_framework/skthemes/metabox/');
...
/** original code
			...
			if ( $field['type'] == "title" ) {
				...
				if( $this->_meta_box['show_names'] == true ) {
					echo '<th style="width:24%"><label for="', $field['id'], '">', $field['name'], '</label></th>';
				}			
				echo '<td>';
			}		
**/

//mod by denzel, to allow more space for label in type text_small, and checkbox_small changing th width to 65px.
//@since version 3.0

			if ( $field['type'] == "title" ) {
				echo '<td colspan="2">';
			
			} elseif($field['type'] == "text_small" && $this->_meta_box['show_names'] == true){
			
					echo '<th style="width:65px"><label for="', $field['id'], '">', $field['name'], '</label></th>';		
				    echo '<td>';			
			}
			
			
			elseif($field['type'] == "checkbox_small" && $this->_meta_box['show_names'] == true){
			
					echo '<th><label style="text-align:left !important;" for="', $field['id'], '">', $field['name'], '</label></th>';		
				    echo '<td>';			
			}			
...
/*
* mod by denzel, added checkbox_small type for side panel
*/					
				case 'checkbox_small':
					echo '<input style="position:relative;left:-80px;" type="checkbox" name="', $field['id'], '" id="', $field['id'], '"', $meta ? ' checked="checked"' : '', ' />';
					echo '<span class="cmb_metabox_description">', $field['desc'], '</span>';
					break;					
/*
* end of mod additional by denzel
*/					
