<?php
function skthemes_manage_javascripts_scripts(){
...
/* Register Scripts
/*-----------------------------------------------------------------*/
wp_register_script( 'skthemes-custom', SKTTHEMES_JS .'/skthemes.js', array('jquery'),'2.0',$in_footer = true);
wp_register_script( 'jquery-cycle-all', SKTTHEMES_JS .'/jquery.cycle.all.min.js', array('jquery'),'2.9.4',$in_footer = true);
wp_register_script( 'jquery-ui', SKTTHEMES_JS .'/jquery-ui-1.8.15.custom.min.js', array('jquery'),'1.8.15',$in_footer = true);
wp_register_script( 'pretty-photo', SKTTHEMES_JS .'/jquery.prettyPhoto.js', array('jquery'),'1.0',$in_footer = true);
wp_register_script( 'comment-reply', site_url().'/wp-includes/js/comment-reply.js',$deps=null,'1.0',$in_footer = true);
wp_register_script( 'superfish', SKTTHEMES_JS .'/superfish.js', array('jquery'),'1.0',$in_footer = true);
wp_register_script( 'hoverintent', SKTTHEMES_JS .'/hoverIntent.js', array('jquery'),'1.0',$in_footer = true);
...
wp_register_script( 'pretty-sociable', SKTTHEMES_JS .'/jquery.prettySociable.js', array('jquery'),'2.9.4',$in_footer = true);
...
wp_enqueue_script( 'skthemes-custom');
...
wp_register_script('skthemes-woocommerce', SKTTHEMES_JS .'/custom-woocommerce.js', array('jquery'), '',$in_footer = true);
wp_enqueue_script('skthemes-woocommerce');
}
		
		
		
global $ttso;
$dragshare = $ttso->skt_dragshare;
...
wp_enqueue_script( 'pretty-sociable', SKTTHEMES_JS .'/jquery.prettySociable.js', array('jquery'),'1.2.1',$in_footer = true);		
add_action('wp_footer','skthemes_hook_footer_scripts');			
...
add_action('template_redirect', 'skthemes_manage_javascripts_scripts',90);
...
*
*/
function skthemes_hook_footer_scripts(){

	//get option values
    global $ttso;
	$jcycle_timeout = $ttso->skt_jcycle_timeout; // jQuery banner cycle time
	$jcycle_pause_hover = $ttso->skt_jcycle_pause_hover; //whether pause on hover.
...
global $ttso;
$testimonial_enable = $ttso->skt_testimonial_enable;

if($testimonial_enable == "true"){
$testimonial_timeout = $ttso->skt_testimonial_timeout;
$testimonial_pause_hover = $ttso->skt_testimonial_pause_hover;
