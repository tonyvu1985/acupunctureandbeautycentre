...
class pb_backupbuddy_backup {
	...
	function process_backup( $serial, $trigger = 'manual' ) {
		...
		if ( $response === false ) { // Function finished but reported failure.
			...
			if ( pb_backupbuddy::$options['log_level'] == '3' ) { // Only send log in email if full logging enabled.
				$log_file = pb_backupbuddy::$options['log_directory'] . 'status-' . $serial . '_' . pb_backupbuddy::$options['log_serial'] . '.txt';
