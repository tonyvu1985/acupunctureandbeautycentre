...
class pb_backupbuddy_core {
	 ...
	 *	@return		null
	 */
	function mail_error( $message, $override_recipient = '', $attachments = array() ) {
	 ...
	 *
	 */
	public function get_remote_send_defaults() {
		return array(
			'destination'		=>	0,
			'file'				=>	'',
			'file_size'			=>	0,
	...
	function send_remote_destination( $destination_id, $file, $trigger = '', $send_importbuddy = false, $delete_after = false ) {
		...
		require_once( pb_backupbuddy::plugin_path() . '/classes/fileoptions.php' );
		$fileoptions_obj = new pb_backupbuddy_fileoptions( pb_backupbuddy::$options['log_directory'] . 'fileoptions/send-' . $identifier . '.txt', $read_only = false, $ignore_lock = true, $create_file = true );
		...
		require_once( pb_backupbuddy::plugin_path() . '/classes/fileoptions.php' );
		$fileoptions_obj = new pb_backupbuddy_fileoptions( pb_backupbuddy::$options['log_directory'] . 'fileoptions/send-' . $identifier . '.txt', $read_only = false, $ignore_lock = false, $create_file = false );
		if ( true !== ( $result = $fileoptions_obj->is_ok() ) ) {
	...
	public function trim_remote_send_stats() {
		...
		$send_fileoptions = pb_backupbuddy::$filesystem->glob_by_date( pb_backupbuddy::$options['log_directory'] . 'fileoptions/send-*.txt' );
