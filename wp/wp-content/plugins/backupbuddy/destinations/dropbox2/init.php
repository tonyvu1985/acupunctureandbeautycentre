...
class pb_backupbuddy_destination_dropbox2 { // Ends with destination slug.
	...
	public static $destination_info = array(
		'name'			=>		'Dropbox <small>(PHP v5.3+)</small>',
		'description'	=>		'Dropbox.com support for servers running PHP v5.3 or newer. Improved functionality and reliability compared to the older version.',
	);
	...
	public static function send( $settings = array(), $files = array(), $send_id = '' ) {
		...
		if ( $settings['_chunk_upload_id'] != '' ) {
			...
			if ( ( $chunked_destination_settings['_chunk_upload_id'] != '' ) || ( count( $files ) > 0 ) ) {
				...
				$schedule_result = pb_backupbuddy::$classes['core']->schedule_single_event( time(), pb_backupbuddy::cron_tag( 'destination_send' ), array( $chunked_destination_settings, $files, $send_id ) );
		...
		foreach( $files as $file_id => $file ) {
			...
			if ( ( $settings['max_chunk_size'] >= 5 ) && ( ( $file_size / 1024 / 1024 ) > $settings['max_chunk_size'] ) ) { // chunked send.
				...
				pb_backupbuddy::$classes['core']->schedule_single_event( time(), pb_backupbuddy::cron_tag( 'destination_send' ), array( $chunked_destination_settings, $files, $send_id ) );
				spawn_cron( time() + 150 ); // Adds > 60 seconds to get around once per minute cron running limit.
				update_option( '_transient_doing_cron', 0 ); // Prevent cron-blocking for next item.
