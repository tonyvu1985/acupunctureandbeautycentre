...
class pb_backupbuddy_destination_sftp {
	...
	public static $destination_info = array(
		'name'			=>		'sFTP (Beta)',
		'description'	=>		'Beta support for Secure File Transport Protocol (over SSH). This is more secure way of sending files between servers than FTP by using SSH. Web hosting accounts are more frequently providing this feature for greater security. This implementation is fully in PHP so PHP memory limits may be a limiting factor on some servers.',
	);
				...
				if ( $settings['archive_limit'] > 0 ) {
					...
					if ( ( count( $backups ) ) > $settings['archive_limit'] ) {
						...
						if ( $delete_fail_count != 0 ) {
							pb_backupbuddy::$classes['core']->mail_error( sprintf( __('sFTP remote limit could not delete %s backups. Please check and verify file permissions.', 'it-l10n-backupbuddy' ), $delete_fail_count  ) );
